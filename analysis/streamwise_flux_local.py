...
HT = zeros((len(runs), len(pathtypes), len(components), len(mytau), Nt))
    ...
    for ntau in [4,]:
        ...
        for npath in [2,]:
            ...
            if True:
                ...
                fluxlevs = (arange(-30,30,5)+2.5) * tau/0.2
                fluxticks = arange(-25,26,5) * tau/0.2
                ...
                # just two figures
                fluxlevs = (arange(-25,25,5)+2.5) * tau/0.2
                fluxticks = arange(-20,21,5) * tau/0.2
                ...
                c=contour(b.xc, b.yc, T0, colors='0.5')
                quiver(b.xc[(adens/2)::adens],b.yc[(adens/2):-10:adens],
                    (UTa_div+UTg_div)[(adens/2):-10:adens,(adens/2)::adens],
                    (VTa_div+VTg_div)[(adens/2):-10:adens,(adens/2)::adens], scale=ascale)               
                clabel(c, fmt='%1.1f', fontsize=7)
                #title(r'$\int dz (\bar {\bf v} \bar \theta)_a^{div} \cdot \hat n$')
                ...
                cf=contourf(b.xc, b.yc, Tflux_e_div, fluxlevs, cmap=get_cmap('posneg'), extend='both', rasterized=True);
                c=contour(b.xc, b.yc, T0, colors='0.5')
                qu=quiver(b.xc[(adens/2)::adens],b.yc[(adens/2):-10:adens],
                    UTe_div[(adens/2):-10:adens,(adens/2)::adens],VTe_div[(adens/2):-10:adens,(adens/2)::adens], scale=ascale)
                clabel(c, fmt='%1.1f', fontsize=7)
                #title(r"$\int dz (\bar{ {\bf v'} \theta'})^{div} \cdot \hat n$")
                title(r"$(\bar{ {\bf v'} \theta'})^{div}$")
                #xticks([]); xlabel('x'); yticks([]); ylabel('y')
                ...
                Klevs = arange(0,5000,500) * (tau/0.2)**0.5
                Kticks = arange(0,5000,1000) * (tau/0.2)**0.5
                qscale = ascale
                ...
                # figure(4, figsize=(3.5,2.2))
                # #pcolormesh(b.xc, b.yc, Kdiv, cmap=get_cmap('posneg'), rasterized=True); clim(array([-1,1])*8e3)
                # pc=contourf(b.xc, b.yc, K0, Klevs*2, cmap=get_cmap('jet'), extend='both', rasterized=True);
                # c=contour(b.xc, b.yc, T[0], colors='0.5')
                # clabel(c, fmt='%1.1f', fontsize=7)
                # #quiver(b.xc[(adens/2)::adens],b.yc[(adens/2)::adens],
                # #    UTe_div[(adens/2)::adens,(adens/2)::adens],VTe_div[(adens/2)::adens,(adens/2)::adens], scale=500)
                # title(r"$K_\perp^{div}$ (m$^2$ s$^{-1}$) Surface")
                # xticks([]); xlabel('x'); yticks([]); ylabel('y')
                # #cb=colorbar(pc, cax=axes([0.91,0.1,0.01,0.8]))
                # tight_layout()
                # cb=colorbar(pc, ticks=(2*Kticks))
                ...
                # setp(cl, fontsize=7)
                #             
                # savefig('%s/Kdiv_surf_%s-%s.pdf' % (figdir, avg, r)) 
                
                # flux and diffusivity side by side
                rc('figure.subplot', left=0.09, right=0.88, bottom=0.15, top=0.82, wspace=0.12, hspace=0.12)
                figure(35,figsize=(7,2.75))
                clf()       
                ...
                clabel(c, fmt='%1.1f', fontsize=7)
                #title(r"$\int dz (\bar{ {\bf v'} \theta'})^{div} \cdot \hat n$")
                title(r"${\bf F}^{div}_{eddy}$ (K m$^2$ s$^{-1}$)")
                #xticks([]); xlabel('x'); yticks([]); ylabel('y')
                xticks(array(mytick)*1000, mytick); yticks(array(mytick)*1000, mytick)
                xlabel('x (km)'); ylabel('y (km)')          
                cb=colorbar(cf, ticks=fluxticks)
                cl = getp(cb.ax, 'ymajorticklabels') 
                setp(cl, fontsize=7)
                qk = quiverkey(qu, 0.12, 0.2, 30, r'30 K m$^2$ s$^{-1}$',
                               labelpos='E', coordinates='figure', fontproperties={'size':7})
                ...
                savefig('%s/F_and_K_%s-%s.pdf' % (figdir, avg, r))
            ...
            HT[nrun, npath, 0, ntau] = Qfac*Tflux_a_t
            HT[nrun, npath, 1, ntau] = Qfac*Tflux_g_t
            HT[nrun, npath, 2, ntau] = Qfac*Tflux_e_t
            HT_coords[nrun, npath, ntau] = t
            
            # calc equivalent y
            HT_eqlat[nrun, npath, ntau] = interp(t,T0.mean(axis=1),b.yc)
